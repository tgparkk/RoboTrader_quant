...
def get_candidate_stocks_by_date(db_path: str, date_str: str) -> List[Dict[str, str]]:
    """특정 날짜의 후보 종목 조회"""
    try:
...
def check_date_data(date_str: str):
    """특정 날짜의 데이터 수집 상태 확인"""
    print("=" * 80)
    print(f"[{date_str} 데이터 수집 상태 확인]")
    print("=" * 80)
    ...
    # 해당 날짜의 후보 종목 조회
    candidates = get_candidate_stocks_by_date(str(db_path), date_str)
    
    if not candidates:
        print(f"[경고] {date_display} 날짜의 후보 종목이 없습니다.")
        return
    ...
    for candidate in candidates:
        ...
        # 분봉 데이터 확인
        minute_exists, minute_count, minute_info = check_minute_data(stock_code, date_str)
        if minute_exists and minute_count > 0:
            minute_ok += 1
        
        # 일봉 데이터 확인
        daily_exists, daily_count, daily_info = check_daily_data(stock_code, date_str)
        if daily_exists:
            daily_ok += 1
    ...
    if len(candidates) > 0:
        if minute_ok == len(candidates) and daily_ok == len(candidates):
            ...
            print("[실패] 데이터 수집이 전혀 되지 않았습니다.")
        else:
            print("[부분 성공] 일부 종목의 데이터가 누락되었습니다.")
    print("=" * 80)


if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='특정 날짜의 데이터 수집 상태 확인')
    parser.add_argument('date', type=str, help='확인할 날짜 (YYYYMMDD 또는 YYYY-MM-DD)')
    args = parser.parse_args()
    
    check_date_data(args.date)

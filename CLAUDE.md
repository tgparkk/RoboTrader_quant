...
### 1. 아침 09:30 리밸런싱 (1회 실행)

**위치**: `core/quant/target_profit_loss_calculator.py`

> ⏱️ **2026-05-14 변경**: 리밸런싱 트리거 09:05 → 09:30. 진입시점 멀티버스(`docs/superpowers/reports/2026-05-14-entry-timing-results.md`)에서 14개월 28조합 검증 시 09:30 매수가 09:00/09:05 시초 매수 대비 sharpe 0.34→0.75(+2.2배), 누적수익률 +6.84%→+11.44% 우월 확인. 트리거 윈도우 09:30~09:55 (재시작 catch-up). **TP/SL은 09:00부터 즉시 유지**(독립 안전 메커니즘).
...
- 09:00부터 손절/익절 모두 즉시 허용 (09:30 리밸런싱과 독립 동작)
...
BUY_BLACKLIST = set()                  # 빈 집합 — 블랙리스트 방식 미사용. 058430은 BUY_RET20D_MAX=30 게이트가 자동 차단 (2026-05-07 한시 블랙리스트 제거)
```

> ⚠️ 위 게이트(BUY_RET5D_MIN/MAX, BUY_RET20D_MAX, BUY_MOMENTUM_SCORE_MIN)는 `db_manager.execute_query` 부재로 **2026-03-27~05-10 silent-fail**(무력화)되어 있었음. fix d5bd7c2, **2026-05-11부터 실제 효력**. 백테스트는 cache 기반이라 무관.
...
> ⚠️ **아래 이 절(節) 전체는 (a) V100 전환(2026-04-14) 이전 4팩터 시대, (b) `factor_calculator.py:650` hybrid_score 저장 버그 수정(2026-05-07) 이전에 산출된 수치**다. "팩터 가중치 V25/M30/Q22.5/G22.5", "3.3년 시뮬 460%/258%" 등은 현행 시스템과 무관하며 무효. 슬리피지 0.0025 교정 자체와 "엔진 결정 일치도 96.4%"만 유효.
>
> **현행 정본 수치 (2026-05-07 DB 재생성 후 운영 backtester 멀티버스, 2023~2026 ~3.3년)**:
> - V100 게이트 OFF (TP12/SL6): sharpe **1.93** / +164% / 거래 192
> - **현행 = V100 + TP12/SL6 + M30/R30 게이트: sharpe 2.48 / +237% / 거래 173 — 그 멀티버스 1위**
> - ⚠️ multiverse_min phase9(별개 엔진, V100 단독 sharpe 1.42)의 수치와 혼동 금지
> - 미검증 리스크: 섹터당 보유 한도 (2026-05 건설·소재 동시 손절 사례)
...
1. **09:30 리밸런싱**: 점수 기반 매도/매수 결정 → DB 저장

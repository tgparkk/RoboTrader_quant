...
1️⃣ 계획 수립 (core/quant/quant_rebalancing_service.py)
   ├─ 최신 퀀트 포트폴리오 조회 (어제 15:40 스크리닝 결과)
   ├─ 현재 보유 종목과 비교
   └─ 매도/매수/유지 리스트 생성
...
│  - quant_portfolio (포트폴리오)                 │
│  - quant_factor_scores (팩터 점수)              │
└─────────────────────────────────────────────────┘
            ↓
┌─────────────────────────────────────────────────┐
│           스크리닝 & 분석                        │
...
### Q3. 종목은 어떻게 선정하나요?
**A**: 매일 15:40 퀀트 스크리닝으로 8가지 팩터 점수를 계산하여 상위 30개 선정.
...
portfolio = db_manager.get_quant_portfolio(
    calc_date="20251228",  # 어제 15:40 스크리닝 결과
    limit=30
)

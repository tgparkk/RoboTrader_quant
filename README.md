...
KOSPI + KOSDAQ 전체 종목 스크리닝 → 상위 10개 종목 선정 → 자동 매매
                                                      ↓
                           점수 높은 종목 = 높은 기대 = 공격적 목표 (익절 20%)
                           점수 낮은 종목 = 낮은 기대 = 보수적 목표 (익절 12%)
```
...
복합 점수 = 순위 점수(40%) + 종합 점수(30%) + 모멘텀 점수(30%)

종목 점수에 따라 5단계로 익절/손절 목표를 다르게 설정합니다.
상위 종목은 공격적(20% 익절), 하위 종목은 보수적(12% 익절).
...
if 종목 in 목표_포트폴리오 and 종목 not in 현재_보유:
    ...
    # 5단계 등급별 목표 설정
    복합_점수 = rank(40%) + total_score(30%) + momentum(30%)
    익절률, 손절률 = 등급별_목표(복합_점수)
...
```python
# core/quant/target_profit_loss_calculator.py:79-88
if composite_score >= 80:
    return 0.20, 0.08  # S등급: 20% 익절, 8% 손절
elif composite_score >= 65:
    return 0.17, 0.09  # A등급: 17% 익절, 9% 손절
elif composite_score >= 50:
    return 0.15, 0.10  # B등급: 15% 익절, 10% 손절
elif composite_score >= 35:
    return 0.13, 0.10  # C등급: 13% 익절, 10% 손절
else:
    return 0.12, 0.10  # D등급: 12% 익절, 10% 손절
```
...
│  │  └─ target_profit_loss_calculator.py  # 5단계 익절/손절률
│  └─ helpers/                       # 헬퍼 모듈

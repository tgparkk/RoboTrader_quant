...
- **자동 리밸런싱**: 매일 09:05 포트폴리오 재구성
- **자동 손익절**: 1분마다 조건 체크, 자동 매도
- **자동 데이터 수집**: 15:30 일봉/재무 데이터 수집
...
| 데이터 | 저장 시각 | 용도 |
|--------|----------|------|
| 일봉 (daily_prices) | 15:30 | 퀀트 팩터 계산, 백테스팅 |
| 재무제표 (financial_statements) | 15:30 | PER, ROE 등 계산 |
| 퀀트 포트폴리오 (quant_portfolio) | 15:40 | 내일 리밸런싱 |
...
| 데이터 | 저장 시각 | 테이블 | 용도 |
|--------|----------|--------|------|
| 📊 **일봉 가격** | 15:30 | `daily_prices` | 퀀트 팩터 계산, 백테스팅 |
| 💼 **재무제표** | 15:30 | `financial_statements` | Value/Quality 점수 |
| 📝 **매매 기록** | 즉시 | `virtual_trading_records` | 손익 관리, 포지션 복원 |
| 🎯 **퀀트 점수** | 15:40 | `quant_portfolio`, `quant_factor_scores` | 리밸런싱 |

**특징**:
- **전 영업일 데이터**: 15:30에 "어제까지" 일봉 수집 (백테스팅 일관성)
...
### Q3. 종목은 어떻게 선정하나요?
**A**: 매일 15:40에 8가지 퀀트 팩터 점수를 계산하여 상위 30개 선정.

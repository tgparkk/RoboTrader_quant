...
FIXED_HOLIDAYS = {
    ...
    (10, 9): "한글날",
    (12, 25): "크리스마스",
    (12, 31): "연말 휴장",  # KRX 자체 휴장 (법정공휴일 아님)
}
...
LUNAR_HOLIDAYS = {
    ...
    "2026-09-24": "추석 전날",
    "2026-09-25": "추석",
    "2026-09-26": "추석 다음날",  # 토요일 자연 차단 (일요일 없으므로 대체 없음 — KRX 공식)
...
# 임시 공휴일 + KRX 거래소 휴장일 (대체공휴일, 근로자의 날, 연말 휴장 등)
# 2026-04-28 mom-strategy 백테스트 검증 중 KRX daily_prices와 캘린더 비교로 누락분 발견·추가.
# FIXED_HOLIDAYS / LUNAR_HOLIDAYS와 일부 중복(예: 5/1, 12/31)하나 is_holiday()가 OR 검사라 무해.
# 명시적 기록 차원에서 모두 보존.
SPECIAL_HOLIDAYS = {
    # 2024
    "2024-04-10": "제22대 국회의원 선거",
    "2024-05-01": "근로자의 날 (KRX 휴장)",
    "2024-05-06": "어린이날 대체공휴일 (5/5 = 일)",
    "2024-05-15": "부처님오신날 (음력 4/8)",
    "2024-10-01": "국군의 날 임시공휴일 (2024 부활)",
    "2024-12-31": "KRX 연말 휴장",
    # 2025
    "2025-01-27": "설 연휴 임시공휴일",
    "2025-03-03": "삼일절 대체공휴일 (3/1 = 토)",
    "2025-05-01": "근로자의 날",
    "2025-05-05": "부처님오신날 (5/5 = 어린이날과 겹침)",
...
def is_special_holiday(date: datetime) -> bool:
    """임시 공휴일 여부 확인 (KIS API 런타임 휴장일 포함)"""
    date_str = date.strftime("%Y-%m-%d")

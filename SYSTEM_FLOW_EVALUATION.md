...
08:26:00  시스템 시작
   ↓
08:26:10  DB 연결 및 API 인증
   ↓
08:26:20  보유 종목 복원 (get_virtual_open_positions)
   ↓      - 수량, 매수가, 목표익절률, 손절률 복원
   ↓      - 상태 POSITIONED로 설정
   ↓
...
**예시**:
- 12/26(목) 08:26 실행 → 12/25(수) 데이터 수집
- 12/26(목) 15:30 실행 → 12/25(수) 데이터 수집 (동일)
- 12/23(월) 08:26 실행 → 12/20(금) 데이터 수집 (주말 건너뛰기)
